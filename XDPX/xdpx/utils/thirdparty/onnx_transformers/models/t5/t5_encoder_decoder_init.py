...
class T5EncoderDecoderInitInputs:
    def __init__(self, encoder_input_ids, encoder_attention_mask, decoder_input_ids=None):
        ...
        self.encoder_input_ids: torch.Tensor = encoder_input_ids.type(torch.int32)
        self.encoder_attention_mask: torch.Tensor = encoder_attention_mask.type(torch.int32)
        self.decoder_input_ids: torch.Tensor = decoder_input_ids.type(torch.int32) if decoder_input_ids is not None else None
...
class T5EncoderDecoderInitHelper:
    ...
    def onnxruntime_inference(ort_session, inputs: T5EncoderDecoderInitInputs):
        ...
        if inputs.decoder_input_ids is not None:
            ort_inputs["decoder_input_ids"] = inputs.decoder_input_ids
