...
from xdpx.utils.thirdparty.onnx_transformers.io_binding_helper import TypeHelper
from xdpx.utils.thirdparty.onnx_transformers.torch_onnx_export_helper import torch_onnx_export
from ... import inference_onnx_binding
...
class T5DecoderInputs:
    def __init__(
        ...
        self.decoder_input_ids: torch.Tensor = decoder_input_ids.type(torch.int32)
        self.encoder_attention_mask: torch.Tensor = encoder_attention_mask.type(torch.int32)
...
class T5DecoderHelper:
    ...
    def export_onnx(
        ...
        sequence_length = "1"
        num_heads = str(decoder.config.num_heads)
        hidden_size = str(decoder.config.d_model)
        head_size = str(decoder.config.d_model // decoder.config.num_heads)
    ...
    def onnxruntime_inference(ort_session, inputs: T5DecoderInputs):
        ...
        if inputs.past_key_values:
            ...
            past_names = PastKeyValuesHelper.get_past_names(num_layers)
            for i, past_tensor in enumerate(inputs.past_key_values):
                ort_inputs[past_names[i]] = past_tensor

        ort_outputs = inference_onnx_binding(model_onnx=ort_session, inputs=ort_inputs,
    ...
    def verify_onnx(
        ...
        for (
            ...
            for i in range(2 * model.config.num_layers):
                ...
                max_diff = numpy.amax(numpy.abs(torch_outputs[1][i].cpu().numpy() - ort_outputs_i.cpu().numpy()))
            ...
            if isinstance(model, T5DecoderInit):
                for i in range(2 * model.config.num_layers):
                    ...
                    max_diff = numpy.amax(
                        numpy.abs(torch_outputs[2][i].cpu().numpy() - ort_outputs_i.cpu().numpy())
                    )
