...
from .measurements import decoder_inference, encoder_inference, full_inference_greedy
from .export import T5DecoderONNXFile, T5EncoderONNXFile, T5DecoderTRTEngine, T5EncoderTRTEngine
...
class T5TRTEncoder(TRTHFRunner):
    ...
    def forward(self, input_ids, *args, **kwargs):
        ...
        if is_cpu_mode:
            folded = self.outputs["hidden_states"].cpu()[:bs * input_length * max_length].view(bs, input_length,
                                                                                               max_length)
        else:
...
class T5TRTDecoder(TRTHFRunner):
    ...
    def __init__(
        ...
        super().__init__(trt_engine_file, network_metadata, hf_config, batch_size=batch_size)
        self.max_sequence_length = T5ModelTRTConfig.MAX_SEQUENCE_LENGTH[network_metadata.variant]
        self.vocab_size = T5ModelTRTConfig.VOCAB_SIZE[network_metadata.variant]
        ...
        self.output_shapes = {
            "hidden_states": (self.batch_size, self.max_sequence_length, self.vocab_size)
        }
        self.output_types = {
            "hidden_states": torch.float32
        }
    ...
    def forward(self, input_ids, encoder_hidden_states, *args, **kwargs):
        ...
        if not self.persist_encoder_hidden_states:
            if is_cpu_mode:
                ...
                :bs * encoder_length * max_length] = encoder_hidden_states.flatten()
        ...
        # TODO: Could we construct a Torch tensor using given data_ptr() to avoid this D2D copy?
        if is_cpu_mode:
            folded = self.outputs["hidden_states"].cpu()[:bs * input_length * vocab_size].view(bs, input_length,
                                                                                               vocab_size)
        else:
...
class T5TRT(TRTInferenceCommand):
    ...
    def execute_benchmarking(
        ...
        input_ids = torch.randint(0, T5ModelTRTConfig.VOCAB_SIZE[metadata.variant], (batch_size, input_seq_len))
