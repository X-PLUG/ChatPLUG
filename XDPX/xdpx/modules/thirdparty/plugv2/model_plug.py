...
from .configuration_plug import PlugConfig
...
class TransformerDecoder(nn.Module):  # Decoder
    ...
    def forward(self, state: TransformerDecoderState, tgt: Tensor,
                memory_bank: Tensor, step: int = None, memory_masks: Tensor = None):
        src_words = state.src
        tgt_words = tgt
...
class PlugPreTrainedModel(PreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]]):
        ...
        config = PlugConfig.from_json_file(config_file) if os.path.isfile(config_file) else PlugConfig()
        config.encoder_pth = os.path.join(pretrained_model_name_or_path, config.encoder_pth)
        checkpoint_file = os.path.join(pretrained_model_name_or_path, WEIGHTS_NAME)
        checkpoint = torch.load(checkpoint_file) if os.path.isfile(checkpoint_file) else None
...
class PlugModel(PlugPreTrainedModel):  # Model
    def __init__(self, config, checkpoint=None):
        ...
        if config.encoder == 'bert' or config.encoder == 'zh_bert':
            self.bert = BertModel(BertConfig.from_pretrained(config.encoder_pth))
        elif config.encoder == 'roberta':
            self.bert = RobertaModel(RobertaConfig.from_pretrained(config.encoder_pth))

        if (config.max_pos > 512):
            my_pos_embeddings = nn.Embedding(config.max_pos, self.bert.model.config.hidden_size)
        ...
        tgt_embeddings = nn.Embedding(self.vocab_size, self.bert.config.hidden_size,
                                      padding_idx=1 if config.encoder == 'roberta' else 0)

        if config.share_emb:
            tgt_embeddings.weight = copy.deepcopy(self.bert.model.embeddings.word_embeddings.weight)
        self.decoder = TransformerDecoder(
            config.dec_layers, config.dec_hidden_size, heads=config.dec_heads,
            d_ff=config.dec_ff_size, dropout=config.dec_dropout, embeddings=tgt_embeddings)
        self.generator = PlugPointerGenerator(config.dec_hidden_size, self.vocab_size)
        ...
        if checkpoint is not None:
            for key in list(checkpoint["model"].keys()):
                ...
                if key.startswith("plug." ):
                    checkpoint["model"][key.replace("plug.", "")] = checkpoint["model"][key]
                    checkpoint["model"].pop(key)
    ...
    def forward(self, src, tgt, mask_src, token_type_ids):
        top_vec, _ = self.bert(src, mask_src, token_type_ids=token_type_ids, return_dict=False)
...
class PlugForConditionalGeneration(PlugPreTrainedModel):
    ...
    def __init__(self, config, checkpoint=None, dataset: str = 'default'):
        ...
        self.config = config
        if config.encoder == 'roberta':
            tokenizer = RobertaTokenizer.from_pretrained(config.encoder_pth, do_lower_case=False)
            ...
            tokenizer = BertTokenizer.from_pretrained(config.encoder_pth, do_lower_case=True)
            symbols = {'BOS': tokenizer.vocab['[CLS]'], 'EOS': tokenizer.vocab['[SEP]'],
                       'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}
        self.tokenizer = tokenizer
        self.symbols = symbols
        self.plug = PlugModel(config, checkpoint)
        self.loss = NMTLossCompute(self.plug.generator, symbols, self.plug.vocab_size, config.label_smoothing)
        # for generation
    ...
    def translate_batch(self, batch: 'Batch', fast: bool = False, *args, **kwargs):
        """
    ...
    def _top_k_top_p_filtering(self, logits, top_k=10, top_p=1.0, filter_value=-float("Inf"), min_tokens_to_keep=1):
        if top_k > 0:
    ...
    def _fast_translate_batch(self,
        ...
        src_features, _ = self.plug.bert(src, mask_src, token_type_ids=token_type_ids, return_dict=False)
        ...
        if bad_words_ids is not None:
            for bw_id in bad_words_ids:
                key = tuple(bw_id[:-1])
                value = bw_id[-1]
                bad_words_prefix_dict[key] = bad_words_prefix_dict.get(key, []) + [value]
                bad_words_prefix_len.add(len(key))

        # Give full probability to the first beam on the first step.
        topk_log_probs = (
            torch.tensor([0.0] + [float("-inf")] * (num_beams - 1),
        ...
        for step in range(max_length):
            ...
            dec_out, attns, state = self.plug.decoder(state, decoder_input, src_features, step=step)

            # Generator forward.
            log_probs = self.plug.generator.forward(dec_out.transpose(0, 1).squeeze(0))
            ...
            if len(bad_words_prefix_dict) > 0:
                ...
                for i in range(num_hypos):
                    ...
                    for pre_len in bad_words_prefix_len:
                        pre_key = tuple(alive_seq[i, step + 1 - pre_len:step + 1].cpu().numpy().tolist())
                        curr_banned_token += bad_words_prefix_dict.get(pre_key, [])
            ...
            if repetition_penalty > 1.0:
                ...
                prev_output_tokens = self.calc_banned_tokens(alive_seq, alive_seq.size(0),
                                                             no_repeat_ngram_size, step + 1)
                # batch_size * num_beams
            ...
            if self.config.sample_topk:
                ...
                topk_ids = topk_ids.view(-1, num_beams)  # (batch_size, 2 * num_beams)
                topk_scores = topk_scores.view(-1, num_beams)  # (batch_size, 2 * num_beams)
            # '''
            else:
                ...
                if (cur_len > 3):
                    for i in range(alive_seq.size(0)):
                        ...
                        if self.config.encoder == "roberta":
                            ...
                            words = self.tokenizer.decode(words).strip().split()
                        else:
                ...
                for i in range(is_finished.size(0)):
                    ...
                    if end_condition[i]:
                        ...
                        if self.config.dataset == "qg_ranking_test" or (
                                self.config.dataset == 'paraphrase' and (not self.config.sample_topk)):
    ...
    def calc_banned_tokens(self, prev_input_ids, num_hypos, no_repeat_ngram_size, cur_len):
    ...
    def translate(self, input_ids: torch.Tensor, attention_mask: torch.Tensor=None, token_type_ids=None, *args, **kwargs) -> Dict[str, torch.Tensor]:
        if attention_mask is None:
            attention_mask = input_ids.ne(self.symbols['PAD']).long()
        batch = self.Batch(batch_size=input_ids.size()[0], src=input_ids, tgt=None, token_type_ids=token_type_ids,
