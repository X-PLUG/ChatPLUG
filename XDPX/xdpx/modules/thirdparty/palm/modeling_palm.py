...
class PalmEmbeddings(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.LayerNorm = PalmLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
  ...
  def forward(self,
    ...
    if checkpoint_activations:
      ...
      for i, layer_module in enumerate(self.layer):
        ...
        # output_all_encoded_layers=True: all layers
        # output_all_encoded_layers=False: last layers
...
class PalmDecoderLayer(nn.Module):
  def __init__(self, config):
    ...
    self.input_layernorm = PalmLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    self.post_attention_layernorm = PalmLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    self.post_cross_attention_layernorm = PalmLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class PalmDecoder(nn.Module):
  def __init__(self, config):
    ...
    self.final_layernorm = PalmLayerNorm(config.hidden_size, eps=config.layer_norm_eps)

  def forward(self, hidden_states, enc_hidden_states, enc_attn_mask, dec_attn_mask,
              checkpoint_activations=False,
              is_infer=False):
...
class PalmDecoderModel(PalmPreTrainedModel):
  ...
  def forward(self,
    ...
    # TODO(junfeng): obtain attention mask

    # obtain encoder attention mask using encoder_hidden_states
    if encoder_attention_mask is None:
      bsz, seqlen, dim = encoder_hidden_states.shape
      encoder_attention_mask = torch.ones((bsz, seqlen), device=input_ids.device)
    # We create a 3D attention mask from a 2D tensor mask.
    # Sizes are [batch_size, 1, 1, to_seq_length]
    # So we can broadcast to [batch_size, num_heads, from_seq_length, to_seq_length]
    # this attention mask is more simple than the triangular masking of causal attention
    ...
    embedding_output = embeddings(input_ids)
    # ic('begin decoder')
    # ic(embedding_output.shape)
    # ic(encoder_hidden_states.shape)
    # ic(extended_attention_mask.shape)
    # ic(attention_mask.shape) None
    # ic(extended_attention_mask)
    # ic(attention_mask)
    # ic(next(self.decoder.parameters()).dtype)
    encoder_hidden_states = self.decoder(embedding_output,
                                         encoder_hidden_states,
                                         extended_attention_mask,
                                         attention_mask,
                                         checkpoint_activations=checkpoint_activations)
    # ic(encoder_hidden_states)
    # TODO(junfeng): change hard code
    return_dict = None
    if not return_dict:
...
class PalmModel(PalmPreTrainedModel):
  ...
  def forward(self,
    ...
    # sequence_output is None is for inference
