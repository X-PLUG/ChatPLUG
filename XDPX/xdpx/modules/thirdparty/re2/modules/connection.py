...
"""
Difference:
- residual order is shifted to match the pytorch implementation
"""
...
def augmented_residual(x, res, i):
    if i == 1:
        x = tf.concat([x, res], axis=-1)  # res is embedding
    elif i > 1:
        hidden_size = int(x.shape[-1])
        x = (res[:, :, :hidden_size] + x) * math.sqrt(0.5)
        x = tf.concat([x, res[:, :, hidden_size:]], axis=-1)  # former half of res is embedding
    return x
