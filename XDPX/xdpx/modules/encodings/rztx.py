...
from ..alignments import MultiheadAttention
from ..activations.gelu import gelu


class RZTXEncoderLayer(nn.Module):
    r"""
    Reference: https://github.com/majumderb/rezero
    Reference Date: April 8, 2020
    ...
    """
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, attention_dropout=0.1, activation='relu'):
        super().__init__()
        if version.parse(torch.__version__) < version.parse('1.3'):
            raise ImportError('RZTXLayer needs pytorch>=1.3 to run.')
